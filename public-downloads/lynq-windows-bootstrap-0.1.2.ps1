...
# Public GitHub artifacts. Fail-closed hash verify before install.
# Pairing code: env LYNQ_PAIRING_CODE only. Pair URL: env LYNQ_PAIR_URL only.
# Never argv. Never logged. Does not disable Windows security.
...
function Assert-PublicUrl([string]$Url) {
  $u = $Url.ToLowerInvariant()
  if ($u -match 'tailscale|tail[0-9]|ts\.net|127\.0\.0\.1|localhost|100\.\d+\.\d+\.\d+') {
    Fail 'PRIVATE_OR_TAILNET_URL'
  }
}
...
if (-not $pairUrl.ToLowerInvariant().StartsWith('https://')) { Fail 'TLS_REQUIRED' }

Assert-PublicUrl $ManifestUrl
$manifest = Invoke-RestMethod -Uri $ManifestUrl -UseBasicParsing
if ($manifest.pe_sha256 -ne $ExpectedPe) { Fail 'MANIFEST_PE_HASH_MISMATCH' }
Assert-PublicUrl ([string]$manifest.pe_url)
Assert-PublicUrl ([string]$manifest.bootstrap_url)
